...
def test_added_change_shape():
    ...
    assert change["after"] == _bundle_after()["proofs"][-1]["proof"]


def test_removed_change_shape():
    after = evolution_checkpoint_bundle([{"id": "a", "proof": _proof_head()}])
...
def test_changed_change_shape():
    ...
    assert change["before"] == _bundle_before()["proofs"][-1]["proof"]
    assert change["after"] == after["proofs"][-1]["proof"]
...
def test_unicode_ids_sort_by_code_point():
    before = evolution_checkpoint_bundle([{"id": "é", "proof": _proof_head()}])
    after = evolution_checkpoint_bundle(
        [
            {"id": "é", "proof": _proof_head()},
            {"id": "a", "proof": _proof_middle()},
        ]
    )
    report = evolution_checkpoint_bundle_diff(before, after)
...
def test_equal_valued_inputs_are_byte_identical_regardless_of_dict_order():
    ...
    second = evolution_checkpoint_bundle_diff(
...
def test_caller_member_order_does_not_change_result():
    ...
    before = evolution_checkpoint_bundle(items)
    after = evolution_checkpoint_bundle(list(reversed(items)))
    first = evolution_checkpoint_bundle_diff(before, _bundle_after())
    second = evolution_checkpoint_bundle_diff(after, _bundle_after())
    assert first == second
...
def test_structurally_invalid_member_proof_raises():
    ...
    before["proofs"][0]["proof"] = {"start": AT1}
    with pytest.raises(ValueError):
        evolution_checkpoint_bundle_diff(before, _bundle_after())
...
def test_false_member_digest_raises():
    ...
    with pytest.raises(ValueError):
        evolution_checkpoint_bundle_diff(_bundle_before(), after)
...
def test_false_member_proof_commitment_raises():
    before = copy.deepcopy(_bundle_before())
...
def test_semantic_forgery_in_member_raises():
    before = copy.deepcopy(_bundle_before())
    before["proofs"][0]["proof"]["stages"][0]["bundle"]["reports"][0]["report"][
        "matrix"
...
def test_http_diff_returns_200_compact_json():
    ...
    assert response.json() == evolution_checkpoint_bundle_diff(before, after)
