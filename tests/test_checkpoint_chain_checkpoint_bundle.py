...
def test_proofs_are_normalized_deep_copies():
    ...
    assert by_id["alpha"]["proof"] == _window_proof(RT1, RT1)
    assert by_id["beta"]["proof"] == _window_proof(RT2, RT3)
    assert by_id["伽马"]["proof"] == _window_proof(RT3, RT3)
    for item in items:
...
def test_single_item_bundle():
    bundle = checkpoint_chain_checkpoint_bundle(
        [{"id": "only", "proof": _window_proof(RT2, RT2)}]
    )
...
def test_window_proof_without_outside_chain_is_accepted():
    # A middle single-stage window is self-contained thanks to its carried
    # front-side diff and must bundle without any access to the full chain.
    bundle = checkpoint_chain_checkpoint_bundle(
...
def test_duplicate_ids_raise():
    with pytest.raises(ValueError):
        checkpoint_chain_checkpoint_bundle(
            [
                {"id": "a", "proof": _window_proof(RT1, RT1)},
                {"id": "a", "proof": _window_proof(RT2, RT2)},
            ]
        )
...
def test_structurally_invalid_proof_raises():
    with pytest.raises(ValueError):
        checkpoint_chain_checkpoint_bundle(
            [{"id": "a", "proof": {"start": RT1}}]
        )


def test_proof_missing_key_raises():
    proof = dict(_window_proof(RT1, RT1))
    del proof["anchor"]
    with pytest.raises(ValueError):
