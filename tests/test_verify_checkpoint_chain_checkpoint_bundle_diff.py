...
DIFF_VERIFY_PATH = BUNDLE_PATH + "/diff/verify"
...
def _expected(report, before_ids=None, after_ids=None):
    ...
    return {
        ...
        "before_proof_ids": before_ids,
        "after_proof_ids": after_ids,
...
def test_verifies_added_and_removed_diff():
...
def test_false_window_proof_in_before_bundle_returns_false():
    report = _diff()
...
def test_false_change_proof_stage_chain_returns_false():
    report = _diff()
    added = next(c for c in report["changes"] if c["id"] == "c")
...
def test_false_change_proof_anchor_returns_false():
    # The added single-stage mid/tail window anchors at a real prior stage
    # digest; replace it with an unsupported but format-valid value.
    report = _diff()
    added = next(c for c in report["changes"] if c["id"] == "c")
...
def test_extra_change_for_unchanged_member_returns_false():
    ...
    # A legal list (unique, sorted, correct kinds and sides) that simply
    # invents a change for the unchanged member a.
    report["changes"] = [
        {
            "id": "a",
            "kind": "changed",
...
def test_bad_change_kind_raises():
    report = _diff()
...
def test_changed_change_with_null_side_raises():
    report = _diff()
    changed = next(c for c in report["changes"] if c["id"] == "b")
    changed["after"] = None
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle_diff(report, _expected(report))
...
def test_change_proof_not_equal_to_bundle_member_raises():
    report = _diff()
    added = next(c for c in report["changes"] if c["id"] == "c")
    # A different but structurally valid window proof on the added after side.
    added["after"] = copy.deepcopy(_proof_a())
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle_diff(report, _expected(report))


def test_wrong_kind_for_member_presence_raises():
    # Member b is present in both bundles with a changed proof, not removed.
    report = _diff()
    changed = next(c for c in report["changes"] if c["id"] == "b")
...
def test_unsorted_extra_change_for_unchanged_member_raises():
    report = _diff()
    unchanged = next(
        member for member in report["after"]["proofs"] if member["id"] == "a"
    )
    # Appended after c: the id order a-after-c violates the strict sort.
...
def test_change_for_unknown_id_raises():
    report = _diff()
    change = copy.deepcopy(next(c for c in report["changes"] if c["id"] == "c"))
    change["id"] = "zzz"
