...
def _items():
    return [
        {"id": "beta", "proof": _proof_middle()},
        {"id": "alpha", "proof": _proof_head()},
        {"id": "伽马", "proof": _proof_single()},
    ]
...
def test_members_sorted_by_id_code_point_regardless_of_caller_order():
    ...
    shuffled = evolution_checkpoint_bundle(list(reversed(_items())))
...
def test_proofs_are_normalized_deep_copies():
    ...
    assert by_id["alpha"]["proof"] == _proof_head()
    assert by_id["beta"]["proof"] == _proof_middle()
    assert by_id["伽马"]["proof"] == _proof_single()
    for item in items:
...
def test_equal_valued_inputs_are_byte_identical():
    ...
    second = evolution_checkpoint_bundle(copy.deepcopy(_items()))
...
def test_structurally_invalid_proof_raises():
    with pytest.raises(ValueError):
        evolution_checkpoint_bundle([{"id": "a", "proof": {"root": "0" * 64}}])
...
def test_http_bundle_returns_200_compact_json():
    ...
    expected = evolution_checkpoint_bundle(_items())
