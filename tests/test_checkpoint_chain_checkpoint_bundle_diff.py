...
def test_added_and_removed_sorted_by_unicode_code_point():
    # before has beta, after has delta (alpha in both): removed beta sorts
    # before added delta.
    before = checkpoint_chain_checkpoint_bundle(
        [
            {"id": "alpha", "proof": _window_proof(RT1, RT1)},
            {"id": "beta", "proof": _window_proof(RT2, RT3)},
        ]
    ...
    after = checkpoint_chain_checkpoint_bundle(
        [
            {"id": "alpha", "proof": _window_proof(RT1, RT1)},
            {"id": "delta", "proof": _window_proof(RT3, RT3)},
        ]
    )
    report = checkpoint_chain_checkpoint_bundle_diff(before, after)
    assert [(c["id"], c["kind"]) for c in report["changes"]] == [
...
def test_caller_member_order_does_not_change_result():
    ...
    before = checkpoint_chain_checkpoint_bundle(items)
    after = checkpoint_chain_checkpoint_bundle(list(reversed(items)))
    assert (
        checkpoint_chain_checkpoint_bundle_diff(before, _bundle_after())
        == checkpoint_chain_checkpoint_bundle_diff(after, _bundle_after())
    )
...
def test_semantic_forgery_in_window_proof_raises():
    before = copy.deepcopy(_bundle_before())
    before["proofs"][0]["proof"]["stages"][0]["bundle"]["proofs"][0]["proof"][
        "stages"
...
def test_http_diff_returns_200_compact_json():
    ...
    assert response.json() == checkpoint_chain_checkpoint_bundle_diff(before, after)
