...
@pytest.mark.parametrize(
    "start,end",
    [(DT1, DT3), (DT1, DT1), (DT1, DT2), (DT2, DT3), (DT2, DT2), (DT3, DT3)],
)
def test_verifies_genuine_windows(start, end):
    proof = _proof(start, end)
    assert verify_bundle_evolution_checkpoint(proof, _expected(proof)) is True
...
def test_verification_does_not_mutate_inputs():
    ...
    assert verify_bundle_evolution_checkpoint(proof, expected) is True
...
def test_proof_not_a_dict_raises():
    with pytest.raises(ValueError):
        verify_bundle_evolution_checkpoint([], _expected(_proof(DT1, DT1)))
...
def test_proof_wrong_top_level_keys_raises():
    ...
    proof["extra"] = 1
    with pytest.raises(ValueError):
        verify_bundle_evolution_checkpoint(proof, _expected(_proof(DT1, DT1)))


def test_proof_missing_top_level_key_raises():
    proof = dict(_proof(DT2, DT3))
    del proof["anchor"]
    with pytest.raises(ValueError):
        verify_bundle_evolution_checkpoint(proof, {})
...
def test_expected_not_a_dict_raises():
    with pytest.raises(ValueError):
        verify_bundle_evolution_checkpoint(_proof(DT1, DT1), [])
...
def test_expected_wrong_keys_raises():
    ...
    with pytest.raises(ValueError):
        verify_bundle_evolution_checkpoint(proof, expected)
...
def test_wrong_anchor_returns_false():
    ...
    assert verify_bundle_evolution_checkpoint(proof, expected) is False
...
def test_declared_commitment_false_returns_false():
    ...
    assert (
        verify_bundle_evolution_checkpoint(proof, _expected(_proof(DT2, DT3)))
        is False
    )
...
def test_false_stage_digest_returns_false():
    ...
    proof["stages"] = stages
    assert (
        verify_bundle_evolution_checkpoint(proof, _expected(_proof(DT1, DT3)))
        is False
    )
...
def test_false_carried_diff_bundle_summary_returns_false():
    ...
    proof["stages"] = stages
    assert (
        verify_bundle_evolution_checkpoint(proof, _expected(_proof(DT2, DT2)))
        is False
    )
...
def test_inner_diff_extra_change_returns_false():
    # A legal change entry for "alpha", a member present and unchanged on
    # both sides, is structurally valid but makes the legal change list
...
def test_skipped_stage_returns_false():
    ...
    genuine = _proof(DT1, DT3)
    proof = dict(_proof(DT2, DT3))
    assert len(proof["stages"]) == 2
    assert (
        verify_bundle_evolution_checkpoint(proof, _expected(genuine))
        is False
    )
...
def test_http_verify_false_summary_returns_200_false():
    ...
    proof["commitment"] = "f" * 64
    response = client.post(
        VERIFY_PATH, json={"proof": proof, "expected": _expected(_proof(DT2, DT3))}
    )
    assert response.status_code == 200
