...
def _bundle():
    return checkpoint_chain_checkpoint_bundle(_items())
...
def test_verifies_genuine_bundle():
    bundle = _bundle()
    assert (
        verify_checkpoint_chain_checkpoint_bundle(bundle, _expected(bundle)) is True
    )


def test_verifies_without_full_chain_or_history():
    # Every embedded window proof (including the middle single-stage window
    # carrying only a front-side diff) self-verifies from the bundle alone.
    bundle = checkpoint_chain_checkpoint_bundle(
        [{"id": "solo", "proof": _window_proof(RT2, RT2)}]
    )
    assert (
        verify_checkpoint_chain_checkpoint_bundle(bundle, _expected(bundle)) is True
    )
...
def test_verification_does_not_mutate_inputs():
    ...
    assert verify_checkpoint_chain_checkpoint_bundle(bundle, expected) is True
...
def test_report_not_a_dict_raises():
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle([], _expected(_bundle()))
...
def test_report_wrong_top_level_keys_raises():
    ...
    bundle["extra"] = 1
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle(bundle, _expected(_bundle()))
...
def test_report_missing_top_level_key_raises():
    ...
    del bundle["root"]
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle(bundle, {})
...
def test_duplicate_member_ids_raise():
    ...
    # Reorder-adjustment is unnecessary: duplication alone must raise.
    bundle["proofs"] = proofs
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle(bundle, {})
...
def test_expected_not_a_dict_raises():
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle(_bundle(), [])
...
def test_expected_wrong_keys_raises():
    ...
    with pytest.raises(ValueError):
        verify_checkpoint_chain_checkpoint_bundle(bundle, expected)
...
def test_wrong_expected_root_returns_false():
    ...
    expected = dict(_expected(bundle))
    expected["root"] = "f" * 64
    assert verify_checkpoint_chain_checkpoint_bundle(bundle, expected) is False
