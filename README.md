...
- `regulation_policy_compiler.history.verify_audit(report, expected)`：纯函数校验哈希链审计报告，不访问历史/文件且不改输入。`report` 恰含 `id,start,end,root,entries`，`expected` 恰含 `id,start,end,root`；`id` 为非空字符串，`start`/`end` 为合法 UTC 秒且 `start<=end`，`root` 及各项 `previous`/`digest` 为 64 位小写十六进制字符串；`entries` 为非空数组，项恰含 `at,decision,trace,basis,previous,digest`（前四项沿用 `audit` 契约），`at` 严格递增且落在闭区间 `[start,end]`；上述非法均抛 `ValueError`。两者对应字段不同返回 `False`；否则规范化 `basis` 及 `when` 键序后按 `audit` 定义的 `C`、UTF-8、SHA-256 逐项重算链（首项 `previous` 为 64 个 `0`，后项等于前项 `digest`），链、`digest` 或 `root` 校验失败返回 `False`，全部通过返回 `True`。
