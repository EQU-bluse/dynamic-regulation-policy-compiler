...
- `regulation_policy_compiler.policy.policy_schedule(start, end, rules)`：`start`、`end` 须为合法 UTC 秒且 `start<=end`，`rules` 校验同 `compile_rules`，任一非法抛 `ValueError` 且不改输入。候选时点为 `start` 及规则落在 `(start,end]` 的 `from` 与非空 `to`，去重升序后逐点编译；首点必保留，后续计算 `policy_delta(上一保留点时间, 当前候选时间, rules)`，若其 `rules=[]` 且 `conflicts` 的 `added`、`removed` 均为空则丢弃，否则保留。返回深副本，顶层键序 `start,end,points`；点键序 `at,rules,conflicts,delta`，`rules`、`conflicts` 取该时点 `compile_rules` 同名值，首点 `delta=null`，后续 `delta` 为上述 `policy_delta` 完整结果。
