...
- `regulation_policy_compiler.policy.decision_impact(from_at, to_at, cases, rules)`：`from_at`、`to_at` 须为合法 UTC 秒且 `from_at<=to_at`，`rules` 校验同 `compile_rules`；`cases` 为列表，项恰含 `id,facts`，`id` 为非空且互不相同的字符串，`facts` 校验同 `explain`；任一非法抛 `ValueError` 且不改输入，空 `cases` 合法。返回深副本，顶层键序 `from,to,policy,cases`；`policy` 为 `policy_delta` 完整结果；`cases` 按 `id` Unicode 码点升序，项键序 `id,changes,before,after`，`before`/`after` 为两时点 `explain` 完整结果，`changes` 按 `decision,trace,basis,conflicts` 顺序列出差异字段，无差异为空。
