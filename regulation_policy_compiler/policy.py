...
def checkpoint_chain_checkpoint(
    ...
    checkpoint-bundle, adjacent-diff, chain-link, stage-digest, or root
    forgery raises ``ValueError`` before anything is produced.

    The proof covers the continuous run of stages from ``start`` through
    ``end`` inclusive, with no stage skipped, reordered, or rewritten.
    Returns a deep copy containing exactly the keys ``start``, ``end``,
    ``anchor``, ``stages``, ``commitment`` in that order. When the window
    begins at the chain's first stage, ``anchor`` is 64 ASCII ``0``
    characters; otherwise it is the immediately preceding stage's ``digest``.
    ``stages`` are independent deep copies of the full window stages, each
    ...
    can be recomputed without the chain outside the window. ``commitment``
    is the last window stage's ``digest`` and also equals the original chain
    ``root`` when the window reaches the chain's end. Equal-valued inputs
    yield byte-identical results and no level shares an input object.
    """
    start = _check_time(start, "start")
    end = _check_time(end, "end")
    if start > end:
...
def _normalize_verified_checkpoint_chain_checkpoint(
    ...
    """Run every structural and semantic check on a chain checkpoint proof.
    ...
    or carries the full front-side diff whose ``after`` bundle reproduces
    the first stage. Window disorder or an adjacent diff that does not
    correspond raises ``ValueError``. False declared summaries and
    incomplete legal change lists are carried back as data
    ...
    for index, stage in enumerate(raw_stages):
        ...
        if index == 0 and previous_digest == _CHECKPOINT_CHAIN_GENESIS:
            ...
            diff_content, before_members, after_members, _changes, diff_complete = (
                _normalize_verified_checkpoint_diff_report(
                    stage["diff"], f"{stage_field}.diff"
                )
            )
...
def verify_checkpoint_chain_checkpoint(proof: Any, expected: Any) -> bool:
    ...
    increasing, whose bundles pass every checkpoint-bundle structural and
    ...
    ``null`` diff; any other first stage keeps its front-side diff whose
    ``after`` bundle reproduces that stage, and every later diff must
    reproduce the preceding and current bundles. ``expected`` must have
    exactly the keys ``start``, ``end``, ``anchor``, ``commitment`` and is
    validated the same way. Exact key sets, times, digest formats, stage
    order, and every embedded credential semantic are checked on both
    inputs first; window disorder or an adjacent diff that does not
    correspond raises ``ValueError``.

    Only afterwards are values recomputed rather than trusted: every bundle
    member chain (including the front-side bundle carried by a non-head
    first-stage diff), the first stage digest seeded from ``anchor`` (the
    carried diff recomputed from its before/after bundles), each later
    ...
    tampered diff, a boundary mismatch, or any ``expected`` disagreement
    after the structures are legal; otherwise ``True``.
    """
