...
def _normalize_verified_checkpoint_diff_changes(
    ...
    corresponding bundle member's proof. Returns ``(entries, complete)``
    with fresh normalized entries carrying canonical deep-copy proofs;
    ``complete`` is whether the list covers exactly the added, removed, and
    changed members with unchanged members omitted. Any per-item structural,
    ordering, or side-value violation raises ``ValueError``; a legal list
    that merely misses an expected change (or lists an extra one) is
    structural data carried back in ``complete`` so it yields ``False``
...
def _normalize_verified_checkpoint_diff_report(
    ...
    (its ``before``/``after`` bundles and change proofs are independent deep
    copies) and ``changes_complete`` records whether the legal change list
    covers exactly the added, removed, and changed members. False summaries
    and incomplete change lists are not detected here — callers recompute
    them afterwards, yielding ``False`` rather than ``ValueError``.
    """
    if not isinstance(value, dict) or set(value) != set(
        _CHECKPOINT_BUNDLE_DIFF_REPORT_KEYS
    ):
    ...
    if after_bundle_root != after_root:
        raise ValueError(f"{field}.after_root must equal {field}.after.root")
    change_entries, changes_complete = _normalize_verified_checkpoint_diff_changes(
        value["changes"], before_members, after_members, f"{field}.changes"
    )
    content = {
        "before_root": before_root,
...
def verify_evolution_checkpoint_bundle_diff(report: Any, expected: Any) -> bool:
    ...
    members. A structurally legal change list that omits an expected change
    or lists an extra one is not a structural error: it yields ``False``
    after validation.
...
_CHECKPOINT_CHAIN_STAGE_ITEM_KEYS = frozenset({"at", "bundle"})
_CHECKPOINT_CHAIN_REPORT_KEYS = ("root", "stages")
_CHECKPOINT_CHAIN_STAGE_KEYS = ("at", "bundle", "diff", "previous", "digest")
_CHECKPOINT_CHAIN_EXPECTED_KEYS = ("root", "stages")
_CHECKPOINT_CHAIN_EXPECTED_STAGE_KEYS = frozenset(
...
def _checkpoint_chain_stage_digest(
    ...
    """Compute one checkpoint chain stage digest.
...
def _checkpoint_bundle_semantics(bundle: dict[str, Any]) -> list[tuple[str, Any]]:
    ...
    Projects a ``root, proofs`` bundle to the ordered list of member ids and
    their summary-free proof semantics, so a bundle embedded inside a stage
    diff can be checked against the neighboring stage bundle on semantics: a
    false member or root summary then yields ``False`` at recomputation
    instead of a structural ``ValueError``.
    """
    return [
        (member["id"], _checkpoint_proof_semantics(member["proof"]))
...
def checkpoint_chain(stages: Any) -> dict[str, Any]:
    ...
    the keys ``at`` and ``bundle``; array order is the release order. Each
    ``at`` must be a valid UTC second strictly increasing (a non-list, an
    empty list, duplicates, disorder, or missing/extra fields raise
    ``ValueError``), and every ``bundle`` must pass every existing
    :func:`evolution_checkpoint_bundle` structural, embedded-proof,
    member-chain, and root check — a false proof, member link, or root
    digest raises ``ValueError``.

    Returns a deep copy with keys ``root``, ``stages`` in that order; each
    stage has keys ``at``, ``bundle``, ``diff``, ``previous``, ``digest``.
    The first stage's ``diff`` is ``null``; every later ``diff`` is the
    complete :func:`evolution_checkpoint_bundle_diff` credential from the
    preceding bundle to the current one under the existing diff semantics.
    The first stage's ``previous`` is 64 ASCII ``0`` characters; every later
    stage's ``previous`` is the preceding stage's ``digest``. The per-stage
    payload contains exactly ``at``, ``bundle``, ``diff`` in that key order,
    encoded as compact UTF-8 JSON (``ensure_ascii=False``,
    ``separators=(',', ':')``) with no newline; ``digest`` is the lowercase
    hexadecimal SHA-256 of ``previous``'s ASCII bytes immediately followed
    by that payload, and ``root`` is the last stage's ``digest``.
...
def _normalize_verified_checkpoint_chain(
    ...
    """Run every structural and semantic check on a checkpoint chain report.

    Enforces the full :func:`checkpoint_chain` report contract: exact
    top-level and per-stage key sets, hex64 ``root``/``previous``/``digest``
    fields, a non-empty strictly increasing ``at`` chain, fully valid
    checkpoint bundles, a ``null`` diff on the first stage and a complete
    structurally valid adjacent checkpoint diff credential on every later
    stage whose embedded bundles reproduce the neighboring stage bundles on
    semantics. Returns ``(root, stages)`` with fresh normalized entries
    (canonical bundle and diff copies). A false declared bundle summary,
    broken bundle chain, or incomplete legal change list is carried back as
    data so it yields ``False`` only after every input validates. Any
    structural or semantic violation raises ``ValueError`` and the input is
    never mutated.
    """
    if not isinstance(report, dict) or set(report) != set(
        _CHECKPOINT_CHAIN_REPORT_KEYS
    ):
        raise ValueError(
            f"{field} must be a dict with exactly the keys root, stages"
        )
    ...
    for index, stage in enumerate(raw_stages):
        ...
        if index == 0:
            ...
            diff_content, _, _, _, diff_complete = (
            ...
            if _checkpoint_bundle_semantics(
                diff_content["before"]
            ) != _checkpoint_bundle_semantics(normalized_stages[-1]["bundle"]):
                raise ValueError(
                    f"{stage_field}.diff.before must reproduce the preceding "
                    "stage bundle"
                )
...
def verify_checkpoint_chain(report: Any, expected: Any) -> bool:
    ...
    ``report`` must be a dict with exactly the keys ``root``, ``stages`` and
    conform level by level to the :func:`checkpoint_chain` contract.
    ``expected`` must have exactly the keys ``root``, ``stages`` with a
    ...
    sets, types, digest formats, strictly increasing timestamps, fully
    valid per-stage bundles and their embedded credentials, a ``null``
    first diff, and each later stage's complete adjacent diff credential
    reproducing its neighbors on semantics — completes on both inputs
    before any comparison; any violation raises ``ValueError``.

    Only afterwards are values recomputed rather than trusted: every
    bundle's embedded proofs and member chain, each adjacent
    :func:`evolution_checkpoint_bundle_diff` credential (its digest
    included), every stage digest over the ``previous`` bytes plus the
    compact JSON of ``at``, ``bundle``, ``diff``, the stage
    ``previous``/``digest`` chain from the 64-zero genesis, the final
    ``root``, and the ``expected`` timestamp/bundle-root/diff-digest
    triples and overall root. Returns ``False`` when any chain link, stage
    digest, bundle, diff, root, or expected value does not match, or a
    stage is skipped; otherwise ``True``.
    """
    report_root, stages = _normalize_verified_checkpoint_chain(report, "report")

    if not isinstance(expected, dict) or set(expected) != set(
        _CHECKPOINT_CHAIN_EXPECTED_KEYS
    ):
        raise ValueError(
            "expected must be a dict with exactly the keys root, stages"
        )
    ...
    for index, stage in enumerate(raw_expected_stages):
        field = f"expected.stages[{index}]"
        if not isinstance(stage, dict) or set(stage) != (
            _CHECKPOINT_CHAIN_EXPECTED_STAGE_KEYS
        ):
            raise ValueError(
                f"{field} must be a dict with exactly the keys "
                "at, bundle_root, diff_digest"
        ...
        if index == 0:
            if diff_digest is not None:
                raise ValueError(
                    f"{field}.diff_digest must be null on the first stage"
                )
        else:
    ...
    previous = _CHECKPOINT_CHAIN_GENESIS
    for index, stage in enumerate(stages):
        expected_at, expected_bundle_root, expected_diff_digest = (
        ...
        if index > 0:
            ...
            if stage["diff"] != recomputed_diff:
                return False
            if recomputed_diff["digest"] != expected_diff_digest:
                return False
        ...
        if stage["digest"] != recomputed_digest:
            return False
        if stage["at"] != expected_at or stage["bundle_root"] != expected_bundle_root:
            return False
        previous = recomputed_digest
    if report_root != previous or report_root != expected_root:
        return False
...
def _canonical_checkpoint_chain_checkpoint_from_normalized(
    normalized: dict[str, Any],
) -> dict[str, Any]:
    """Rebuild a fresh ``start, end, anchor, stages, commitment`` proof.
...
def checkpoint_chain_checkpoint_bundle(items: Any) -> dict[str, Any]:
    ...
    :func:`checkpoint_chain_checkpoint` window contract. Each proof is
    checked with the existing verification semantics before anything is
    produced: any structural or semantic violation — window boundaries, the
    anchor, stage order, embedded bundles, adjacent diffs, member links, or
    the terminal commitment — and any proof that does not recompute as true
    raises ``ValueError``.
...
def _normalize_verified_checkpoint_chain_checkpoint_bundle(
    ...
    ids, and every member proof's full checkpoint-chain-window structural
    and embedded-credential semantic checks. A proof that merely recomputes
    as false is structural data, so it is carried back inside the returned
...
def verify_checkpoint_chain_checkpoint_bundle(report: Any, expected: Any) -> bool:
    ...
    without the full checkpoint chain: every embedded window proof is
    self-verifying.
    ...
    bundle-wide unique non-empty ids; each ``previous`` and ``digest`` must
    be 64 lowercase hexadecimal characters, and every ``proof`` must fully
    pass the existing :func:`verify_checkpoint_chain_checkpoint` structural
    and semantic checks. Both inputs complete every key-set, type,
    digest-format, member-order, and per-proof structural/semantic check
    before any comparison; any violation raises ``ValueError``.
    ...
    previous = _CHECKPOINT_BUNDLE_GENESIS
    for member in members:
        if not _checkpoint_chain_checkpoint_is_truthful(member["proof"]):
            return False
        if member["previous"] != previous:
            return False
        recomputed = _checkpoint_bundle_member_digest(
...
_CHECKPOINT_CHAIN_BUNDLE_DIFF_REPORT_KEYS = _CHECKPOINT_BUNDLE_DIFF_REPORT_KEYS
_CHECKPOINT_CHAIN_BUNDLE_DIFF_CHANGE_KEYS = _CHECKPOINT_BUNDLE_DIFF_CHANGE_KEYS
_CHECKPOINT_CHAIN_BUNDLE_DIFF_EXPECTED_KEYS = _CHECKPOINT_BUNDLE_DIFF_EXPECTED_KEYS
...
def _chain_stage_diff_semantics(diff: dict[str, Any] | None) -> Any:
    ...
    A chain window proof's stage diff is an
    :func:`evolution_checkpoint_bundle_diff` credential: its ``before``/
    ``after`` bundles are evolution checkpoint bundles (``root`` plus
    ``proofs`` members) and each non-null change side is an evolution
    checkpoint proof, so the bundles project through
    :func:`_checkpoint_bundle_semantics` and the change sides through
    :func:`_checkpoint_proof_semantics`. This mirrors
    :func:`_checkpoint_diff_semantics`, which handles the matrix-bundle
    diffs one level down.
    """
    if diff is None:
        return None

    def proof_semantics(proof: dict[str, Any] | None) -> Any:
        return None if proof is None else _checkpoint_proof_semantics(proof)

    return {
        "before": _checkpoint_bundle_semantics(diff["before"]),
        "after": _checkpoint_bundle_semantics(diff["after"]),
        "changes": [
            {
                "id": change["id"],
                "kind": change["kind"],
...
def _chain_checkpoint_proof_semantics(proof: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "stages": [
            {
                "at": stage["at"],
                "bundle": _checkpoint_bundle_semantics(stage["bundle"]),
                "diff": _chain_stage_diff_semantics(stage["diff"]),
            }
            for stage in proof["stages"]
        ],
    }
...
def _chain_checkpoint_bundle_is_truthful(
    ...
    """Recompute a normalized chain window proof bundle's hash chain.
...
def checkpoint_chain_checkpoint_bundle_diff(
    ...
    """Attest the difference between two checkpoint chain window proof bundles.

    Pure function: it only processes its arguments, accesses neither history
    nor files, and never mutates an input object at any level.

    Both ``before`` and ``after`` must be complete
    :func:`checkpoint_chain_checkpoint_bundle` reports. Before anything is
    produced, both sides pass the full
    :func:`verify_checkpoint_chain_checkpoint_bundle` structural and semantic
    checks: key sets, digest formats, unique strictly ascending member ids,
    and every embedded chain window proof; the second side is still fully
    checked when the first side merely carries a false digest. Any structural
    or semantic violation, and any false window proof, member chain link, or
    bundle root on either side, raises ``ValueError``.
    ...
    bundles. Members are matched by ``id``; ``changes`` keeps only the
    ``added`` (only in ``after``), ``removed`` (only in ``before``), and
    ``changed`` (present in both but with a different window proof) members,
    ordered by ``id`` in ascending Unicode code point order. Each change has
    keys ``id``, ``kind``, ``before``, ``after``; the two sides are
    independent deep copies of the chain window proof or ``null``.
...
def _normalize_chain_checkpoint_diff_change_proof(
    ...
    """Validate one non-null change-side chain window proof.
    ...
    if value is None:
        return None
    return _canonical_checkpoint_chain_checkpoint_from_normalized(
        _normalize_verified_checkpoint_chain_checkpoint(value, field)
    )


def _normalize_verified_chain_checkpoint_diff_changes(
    value: Any,
    before_members: list[dict[str, Any]],
    after_members: list[dict[str, Any]],
    field: str,
) -> tuple[list[dict[str, Any]], bool]:
    """Structurally validate a chain window proof bundle diff ``changes`` list.

    Each change must have exactly the keys ``id``, ``kind``, ``before``,
    ``after``; ids non-empty, report-wide unique, and strictly ascending in
    Unicode code point order; ``kind`` one of ``added``, ``removed``,
    ``changed``; each non-null side a fully valid chain window proof. Every
    change must agree with the two bundles — right kind for its id's
    presence, correct null sides, and proof semantics equal to the
    ...
    for index, change in enumerate(value):
        item_field = f"{field}[{index}]"
        if not isinstance(change, dict) or set(change) != (
            _CHECKPOINT_CHAIN_BUNDLE_DIFF_CHANGE_KEYS
        ):
            raise ValueError(
                f"{item_field} must be a dict with exactly the keys "
                "id, kind, before, after"
        ...
        kind = change["kind"]
        if kind not in _CHECKPOINT_CHAIN_BUNDLE_DIFF_KINDS:
            raise ValueError(
                f"{item_field}.kind must be 'added', 'removed', or 'changed'"
            )
        before_proof = _normalize_chain_checkpoint_diff_change_proof(
            change["before"], f"{item_field}.before"
        )
        after_proof = _normalize_chain_checkpoint_diff_change_proof(
            change["after"], f"{item_field}.after"
        )
        before_member = before_by_id.get(member_id)
        after_member = after_by_id.get(member_id)
        if kind == "added":
            ...
            if _chain_checkpoint_proof_semantics(after_proof) != (
                _chain_checkpoint_proof_semantics(after_member["proof"])
            ):
                raise ValueError(
                    f"{item_field}.after does not equal the after bundle's "
            ...
            if _chain_checkpoint_proof_semantics(before_proof) != (
                _chain_checkpoint_proof_semantics(before_member["proof"])
            ):
                raise ValueError(
                    f"{item_field}.before does not equal the before bundle's "
                    f"window proof for id {member_id!r}"
    ...
    for member_id in sorted(set(before_by_id) & set(after_by_id)):
        # Membership in the diff is a semantic question; false summaries are
        # recomputed afterwards and must not fabricate "changed" members.
        if _chain_checkpoint_proof_semantics(
            before_by_id[member_id]["proof"]
        ) != _chain_checkpoint_proof_semantics(after_by_id[member_id]["proof"]):
            expected_ids.add(member_id)
    # Completeness is not a structural property: a legal list that omits an
    # expected change (or adds an extra one) yields False at recomputation.
    return entries, seen == expected_ids
...
def _normalize_verified_chain_checkpoint_diff_report(
    value: Any, field: str
) -> tuple[
    dict[str, Any],
    list[dict[str, Any]],
    ...
    """Run every structural and semantic check on a chain bundle diff report.

    Validates a :func:`checkpoint_chain_checkpoint_bundle_diff`-shaped report
    exactly as :func:`verify_checkpoint_chain_checkpoint_bundle_diff` does:
    ...
    """
    if not isinstance(value, dict) or set(value) != set(
        _CHECKPOINT_CHAIN_BUNDLE_DIFF_REPORT_KEYS
    ):
        raise ValueError(
            f"{field} must be a dict with exactly the keys "
            "before_root, after_root, before, after, changes, digest"
    ...
    change_entries, changes_complete = (
        _normalize_verified_chain_checkpoint_diff_changes(
            value["changes"], before_members, after_members, f"{field}.changes"
        )
    )
    content = {
...
def verify_checkpoint_chain_checkpoint_bundle_diff(
    ...
    """Verify a :func:`checkpoint_chain_checkpoint_bundle_diff` by recomputation.

    Pure function: it only inspects its arguments, touches neither history
    nor files, and never mutates an input object at any level. Every
    embedded window proof is self-verifying, so no full checkpoint chain is
    required.

    ``report`` must be a dict with exactly the keys ``before_root``,
    ``after_root``, ``before``, ``after``, ``changes``, ``digest`` and
    conform level by level to the
    :func:`checkpoint_chain_checkpoint_bundle_diff` contract. The two roots
    and ``digest`` must be 64 lowercase hexadecimal characters, and each
    root must equal the root declared by its embedded bundle.
    ``before``/``after`` must be complete chain window proof bundles that
    pass every structural, member-order, digest-format, and embedded window
    proof semantic check. ``changes`` items must have exactly the keys
    ``id``, ``kind``, ``before``, ``after`` with non-empty unique strictly
    ascending ids, legal kinds, null sides matching each kind, and non-null
    sides that are valid window proofs exactly reproducing the corresponding
    bundle members. A structurally legal change list that omits an expected
    change or lists an extra/unchanged one is not a structural error: it
    yields ``False`` after validation.
    ``expected`` must have exactly the keys ``before_root``, ``after_root``,
    ``before_ids``, ``after_ids``, ``digest``; both id arrays must be
    non-empty lists of unique non-empty strings (caller order is arbitrary;
    comparison is by code point). Any structural, value-domain, uniqueness,
...
_CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_STAGE_ITEM_KEYS = frozenset({"at", "bundle"})
_CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_REPORT_KEYS = ("root", "stages")
_CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_STAGE_KEYS = (
    "at",
    "bundle",
    "diff",
    "previous",
...
_CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_EXPECTED_KEYS = ("root", "stages")
_CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_EXPECTED_STAGE_KEYS = frozenset(
    {"at", "bundle_root", "diff_digest"}
)
...
def _chain_checkpoint_bundle_evolution_stage_digest(
    ...
    """Compute one chain window proof bundle evolution stage digest.
...
def _chain_checkpoint_bundle_semantics(bundle: dict[str, Any]) -> list[tuple[str, Any]]:
    ...
    Projects a ``root, proofs`` chain window proof bundle to the ordered list
    of member ids and their summary-free window-proof semantics, mirroring
    :func:`_checkpoint_bundle_semantics` one level up, so a bundle embedded
    inside an adjacent diff can be checked against the neighboring stage
    bundle on semantics: a false member or root summary then yields
    ``False`` at recomputation instead of a structural ``ValueError``.
    """
    return [
        (member["id"], _chain_checkpoint_proof_semantics(member["proof"]))
        for member in bundle["proofs"]
    ]
...
def checkpoint_chain_checkpoint_bundle_evolution(stages: Any) -> dict[str, Any]:
    ...
    :func:`verify_checkpoint_chain_checkpoint_bundle` checks — every embedded
    window proof (window boundaries, anchor, embedded bundles, adjacent
    diffs, member links, and the terminal commitment), the member chain, and
    the bundle root — or raise ``ValueError``.

    Returns a deep copy with keys ``root``, ``stages`` in that order; each
    stage has keys ``at``, ``bundle``, ``diff``, ``previous``, ``digest``.
    The first stage's ``diff`` is ``null``; every later ``diff`` is the full
    :func:`checkpoint_chain_checkpoint_bundle_diff` credential from the
    preceding bundle to the current one under the existing diff semantics
    (changes judged on the complete normalized window proofs, every non-null
    change side equal to the corresponding bundle member). The first stage's
    ``previous`` is 64 ASCII ``0`` characters; every later stage's
    ``previous`` is the preceding stage's ``digest``. The per-stage payload
    contains exactly ``at``, ``bundle``, ``diff`` in that key order, encoded
    as compact UTF-8 JSON (``ensure_ascii=False``, ``separators=(',', ':')``)
...
def _normalize_verified_checkpoint_chain_checkpoint_bundle_evolution(
    ...
    """Run every structural and semantic check on a bundle evolution report.

    Enforces the full
    :func:`checkpoint_chain_checkpoint_bundle_evolution` report contract:
    exact top-level and per-stage key sets, hex64 ``root``/``previous``/
    ``digest`` fields, a non-empty strictly increasing ``at`` chain, fully
    valid chain window proof bundles, a ``null`` diff on the first stage and
    a complete structurally valid adjacent diff credential on every later
    stage whose embedded bundles reproduce the neighboring stage bundles on
    semantics. Returns ``(root, stages)`` with fresh normalized entries
    (canonical bundle and diff copies). A false declared bundle summary,
    broken bundle chain, or incomplete legal change list is carried back as
    ...
    """
    if not isinstance(report, dict) or set(report) != set(
        _CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_REPORT_KEYS
    ):
        raise ValueError(f"{field} must be a dict with exactly the keys root, stages")
    root = _check_hex64(report["root"], f"{field}.root")
    raw_stages = report["stages"]
    if not isinstance(raw_stages, list) or not raw_stages:
        raise ValueError(f"{field}.stages must be a non-empty list")
    ...
    for index, stage in enumerate(raw_stages):
        ...
        bundle = _canonical_checkpoint_bundle_from_members(bundle_root, bundle_members)
        bundle_truthful = _chain_checkpoint_bundle_is_truthful(
            bundle_root, bundle_members
        )
        ...
        if index == 0:
            ...
            diff_content, _, _, _, diff_complete = (
                _normalize_verified_chain_checkpoint_diff_report(
                    stage["diff"], f"{stage_field}.diff"
                )
            )
            if diff_content["before_root"] != normalized_stages[-1]["bundle_root"]:
            ...
            if _chain_checkpoint_bundle_semantics(
                diff_content["before"]
            ) != _chain_checkpoint_bundle_semantics(normalized_stages[-1]["bundle"]):
                raise ValueError(
                    f"{stage_field}.diff.before must reproduce the preceding "
                    "stage bundle"
                )
            if _chain_checkpoint_bundle_semantics(
                diff_content["after"]
            ) != _chain_checkpoint_bundle_semantics(bundle):
                raise ValueError(
                    f"{stage_field}.diff.after must reproduce this stage bundle"
                )
            diff = diff_content
...
def verify_checkpoint_chain_checkpoint_bundle_evolution(
    ...
    """Verify a :func:`checkpoint_chain_checkpoint_bundle_evolution` report.
    ...
    ``report`` must be a dict with exactly the keys ``root``, ``stages`` and
    conform level by level to the
    :func:`checkpoint_chain_checkpoint_bundle_evolution` contract;
    ``expected`` must have exactly the keys ``root``, ``stages`` with a
    non-empty stage list whose items contain only ``at``, ``bundle_root``,
    and ``diff_digest``: strictly increasing valid ``at`` values, ``root``
    and every ``bundle_root`` 64 lowercase hexadecimal characters, the first
    ...
    per-stage chain window proof bundles and their embedded credentials, a
    ``null`` first diff, and each later stage's complete adjacent
    :func:`checkpoint_chain_checkpoint_bundle_diff` credential reproducing
    its neighbors on semantics — completes on both inputs before any
    comparison; any violation raises ``ValueError``.
    ...
    if not isinstance(expected, dict) or set(expected) != set(
        _CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_EXPECTED_KEYS
    ):
        raise ValueError(
            "expected must be a dict with exactly the keys root, stages"
        )
    ...
    for index, stage in enumerate(raw_expected_stages):
        field = f"expected.stages[{index}]"
        if not isinstance(stage, dict) or set(stage) != (
            _CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_EXPECTED_STAGE_KEYS
        ):
            raise ValueError(
                f"{field} must be a dict with exactly the keys "
                "at, bundle_root, diff_digest"
    ...
    for index, stage in enumerate(stages):
        ...
        if index > 0:
            if not stage["diff_complete"]:
                return False
            recomputed_diff = checkpoint_chain_checkpoint_bundle_diff(
                stages[index - 1]["bundle"], stage["bundle"]
            )
            if stage["diff"] != recomputed_diff:
                return False
            if recomputed_diff["digest"] != expected_diff_digest:
                return False
        recomputed_digest = _chain_checkpoint_bundle_evolution_stage_digest(
            previous, stage["at"], stage["bundle"], stage["diff"]
        )
        if stage["previous"] != previous:
            return False
...
_BUNDLE_EVOLUTION_CHECKPOINT_REPORT_KEYS = (
    "start",
    "end",
    "anchor",
    "stages",
...
_BUNDLE_EVOLUTION_CHECKPOINT_EXPECTED_KEYS = frozenset(
    {"start", "end", "anchor", "commitment"}
)
...
def bundle_evolution_checkpoint(
    ...
    illegal time, a missing boundary, or a reversed window raises
    ``ValueError``). ``report`` must be a complete, fully verifiable
    :func:`checkpoint_chain_checkpoint_bundle_evolution` report: any
    structural, time, ordering, window-proof, member-chain, adjacent-diff,
    stage-link, stage-digest, or root forgery raises ``ValueError`` before
    anything is produced.
    ...
    characters; otherwise it is the immediately preceding stage's ``digest``.
    ``stages`` are independent deep copies of the full window stages, each
    still following the published chain window proof bundle, diff, and
    stage-chain contracts; the first stage of a non-head window keeps its
    original ``diff`` and the bundle on its front side, so even a
    single-stage window can be recomputed without the report outside the
    window. ``commitment`` is the last window stage's ``digest`` and also
    equals the original report ``root`` when the window reaches the report's
    end. Equal-valued inputs yield byte-identical results.
    """
    start = _check_time(start, "start")
    end = _check_time(end, "end")
    if start > end:
...
def _normalize_verified_bundle_evolution_checkpoint(
    ...
    """Run every structural and semantic check on a bundle window proof.

    Enforces the :func:`bundle_evolution_checkpoint` proof contract: exact
    key sets, valid ``start``/``end`` timestamps with ``start <= end``,
    hex64 ``anchor``/``commitment``, a non-empty strictly increasing stage
    chain, fully valid per-stage chain window proof bundles, and adjacent
    diff credentials that reproduce their neighboring bundles. The first
    stage follows the genesis rule (``previous`` of 64 zeroes and ``null``
    diff) or carries the full front-side chain window proof bundle diff
    whose ``after`` bundle reproduces the first stage. Window disorder, an
    illegal change item, or an adjacent diff that does not correspond raises
    ``ValueError``. False declared summaries are carried back as data
    (``bundle_truthful``, ``diff_complete``, and the validated carried-diff
    members) so they yield ``False`` only after every input validates.
    Boundary agreement of ``start``/``end`` with the first/last stage ``at``
    is reported via ``boundaries_ok`` rather than raised. The input is never
    mutated.
    """
    if not isinstance(proof, dict) or set(proof) != set(
        _BUNDLE_EVOLUTION_CHECKPOINT_REPORT_KEYS
    ...
    for index, stage in enumerate(raw_stages):
        ...
        if index == 0 and previous_digest == (
            _CHECKPOINT_CHAIN_BUNDLE_EVOLUTION_GENESIS
        ):
            if stage["diff"] is not None:
                raise ValueError(
            ...
            if _chain_checkpoint_bundle_semantics(
                diff_content["before"]
            ) != _chain_checkpoint_bundle_semantics(preceding["bundle"]):
                raise ValueError(
                    f"{stage_field}.diff.before must reproduce the preceding "
                    "stage bundle"
                )
...
def verify_bundle_evolution_checkpoint(proof: Any, expected: Any) -> bool:
    ...
    UTC seconds with ``start <= end`` and ``anchor``/``commitment`` 64
    lowercase hexadecimal characters. ``stages`` must be a non-empty list of
    full bundle evolution stages whose ``at`` values are strictly
    increasing, whose chain window proof bundles pass every bundle
    structural and embedded-proof semantic check, and whose diffs correspond
    to their neighbors: the genesis stage carries 64 zeroes as ``previous``
    and a ``null`` diff; any other first stage keeps its front-side chain
    window proof bundle diff whose ``after`` bundle reproduces that stage,
    and every later diff must reproduce the preceding and current bundles.
    ``expected`` must have exactly the keys ``start``, ``end``, ``anchor``,
    ``commitment`` and is validated the same way. Exact key sets, times,
    digest formats, stage order, and every embedded credential semantic are
    checked on both inputs first; an illegal structure, disorder, an invalid
    ...
    diff), the first stage digest seeded from ``anchor`` (the carried diff
    recomputed from its before/after bundles and change proofs), each later
    adjacent :func:`checkpoint_chain_checkpoint_bundle_diff` credential,
    every ``previous``/``digest`` stage link, and the terminal
    ``commitment``. Returns ``False`` for a broken link, a wrong anchor, a
    skipped stage, a tampered diff (including a structurally legal change
    list that omits or invents a change), a boundary mismatch, or any
    ``expected`` disagreement after the structures are legal; otherwise
    ``True``.
    """
    normalized = _normalize_verified_bundle_evolution_checkpoint(proof, "proof")
    if not isinstance(expected, dict) or set(expected) != (
        _BUNDLE_EVOLUTION_CHECKPOINT_EXPECTED_KEYS
    ):
        raise ValueError(
            "expected must be a dict with exactly the keys "
            "start, end, anchor, commitment"
    ...
    for stage in stages:
        ...
        if stage["carried_diff"] is not None:
            ...
            if not complete:
                return False
            if not _chain_checkpoint_bundle_is_truthful(
                diff_content["before_root"], before_members
            ):
                return False
            if not _chain_checkpoint_bundle_is_truthful(
            ...
            for change in change_entries:
                for change_proof in (change["before"], change["after"]):
                    if change_proof is not None and not (
                        _checkpoint_chain_checkpoint_is_truthful(change_proof)
                    ):
                        return False
            payload = {
                key: diff_content[key]
                for key in _CHECKPOINT_CHAIN_BUNDLE_DIFF_REPORT_KEYS[:5]
            }
            canonical = json.dumps(
                payload, ensure_ascii=False, separators=(",", ":")
            )
...
def _canonical_bundle_evolution_checkpoint_from_normalized(
    normalized: dict[str, Any],
) -> dict[str, Any]:
    """Rebuild a fresh ``start, end, anchor, stages, commitment`` proof.

    Every level is an independent deep copy of the normalized data produced
    by :func:`_normalize_verified_bundle_evolution_checkpoint`, so the result
    follows the published bundle evolution window proof key order and shares
    no container with the caller's input.
    """
    return {
        "start": normalized["start"],
        "end": normalized["end"],
...
def _bundle_evolution_checkpoint_is_truthful(proof: dict[str, Any]) -> bool:
    ...
    chain through the terminal commitment.
    """
    return verify_bundle_evolution_checkpoint(
        proof,
        {
            "start": proof["start"],
            "end": proof["end"],
...
def bundle_evolution_checkpoint_bundle(items: Any) -> dict[str, Any]:
    ...
    across the list (wrong types, wrong key sets, or duplicates raise
    ``ValueError``), and every ``proof`` must fully conform to the published
    :func:`bundle_evolution_checkpoint` window contract. Each proof is
    checked with the existing verification semantics before anything is
    produced: any structural or semantic violation — window boundaries, the
    anchor, stage order, embedded bundles, adjacent diffs, member links, or
    the terminal commitment — and any proof that does not recompute as true
...
def _normalize_verified_bundle_evolution_checkpoint_bundle(
    ...
    Enforces the full :func:`bundle_evolution_checkpoint_bundle` report
    contract: exact top-level and per-member key sets, hex64
    ``root``/``previous``/``digest`` fields, unique strictly ascending member
    ids, and every member proof's full bundle-evolution-window structural
    and embedded-credential semantic checks. A proof that merely recomputes
    as false is structural data, so it is carried back inside the returned
    canonical member entries and yields ``False`` only after every input
    validates. Returns ``(root, members)`` where each member carries a fresh
...
def verify_bundle_evolution_checkpoint_bundle(report: Any, expected: Any) -> bool:
    ...
    Pure function: it only inspects its arguments, touches neither history
    nor files, and never mutates an input object at any level. It works
    without the full bundle evolution report: every embedded window proof is
    self-verifying.
    ...
    bundle-wide unique non-empty ids; each ``previous`` and ``digest`` must
    be 64 lowercase hexadecimal characters, and every ``proof`` must fully
    pass the existing :func:`verify_bundle_evolution_checkpoint` structural
    and semantic checks. Both inputs complete every key-set, type,
    digest-format, member-order, and per-proof structural/semantic check
    before any comparison; any violation raises ``ValueError``.
    ...
    """
    bundle_root, members = (
        _normalize_verified_bundle_evolution_checkpoint_bundle(report, "report")
    )
...
_BUNDLE_EVOLUTION_CHECKPOINT_BUNDLE_DIFF_REPORT_KEYS = (
    _CHECKPOINT_BUNDLE_DIFF_REPORT_KEYS
)
_BUNDLE_EVOLUTION_CHECKPOINT_BUNDLE_DIFF_CHANGE_KEYS = (
    _CHECKPOINT_BUNDLE_DIFF_CHANGE_KEYS
)
...
def _bundle_evolution_checkpoint_bundle_is_truthful(
    ...
    """Recompute a normalized bundle evolution window proof bundle chain.
...
def _bundle_evolution_stage_diff_semantics(diff: dict[str, Any] | None) -> Any:
    ...
    :func:`_chain_stage_diff_semantics`, which handles the checkpoint
    bundle diffs one level down.
    """
    if diff is None:
        return None
...
def bundle_evolution_checkpoint_bundle_diff(
    ...
    ``null``. Identical bundles yield an empty ``changes`` list.
...
def _normalize_verified_bundle_evolution_checkpoint_bundle_diff_changes(
    ...
    for index, change in enumerate(value):
        ...
        kind = change["kind"]
        if kind not in _BUNDLE_EVOLUTION_CHECKPOINT_BUNDLE_DIFF_KINDS:
            raise ValueError(
                f"{item_field}.kind must be 'added', 'removed', or 'changed'"
            )
        ...
        if kind == "added":
            ...
            if _bundle_evolution_proof_semantics(after_proof) != (
                _bundle_evolution_proof_semantics(after_member["proof"])
            ):
                raise ValueError(
                    f"{item_field}.after does not equal the after bundle's "
                    f"window proof for id {member_id!r}"
            ...
            if _bundle_evolution_proof_semantics(before_proof) != (
                _bundle_evolution_proof_semantics(before_member["proof"])
            ):
                raise ValueError(
                    f"{item_field}.before does not equal the before bundle's "
                    f"window proof for id {member_id!r}"
    ...
    for member_id in sorted(set(before_by_id) & set(after_by_id)):
        # Membership in the diff is a semantic question; false summaries are
        # recomputed afterwards and must not fabricate "changed" members.
        if _bundle_evolution_proof_semantics(
            before_by_id[member_id]["proof"]
        ) != _bundle_evolution_proof_semantics(after_by_id[member_id]["proof"]):
            expected_ids.add(member_id)
    # Completeness is not a structural property: a legal list that omits an
    # expected change (or adds an extra one) yields False at recomputation.
    return entries, seen == expected_ids
...
def _normalize_verified_bundle_evolution_checkpoint_bundle_diff_report(
    ...
    """Run every structural and semantic check on a bundle bundle diff report.

    Validates a :func:`bundle_evolution_checkpoint_bundle_diff`-shaped
    report exactly as
    :func:`verify_bundle_evolution_checkpoint_bundle_diff` does: the six
    exact keys, hex64 summaries, root fields matching their embedded
    bundles, both bundles' full structural and embedded-proof checks, and
    the per-item change consistency rules. Returns ``(content,
    before_members, after_members, change_entries, changes_complete)``
    ...
    if after_bundle_root != after_root:
        raise ValueError(f"{field}.after_root must equal {field}.after.root")
    change_entries, changes_complete = (
        _normalize_verified_bundle_evolution_checkpoint_bundle_diff_changes(
            value["changes"],
            before_members,
            after_members,
            f"{field}.changes",
...
def verify_bundle_evolution_checkpoint_bundle_diff(
    ...
    Pure function: it only inspects its arguments, touches neither history
    nor files, and never mutates an input object at any level. Every
    embedded window proof is self-verifying, so no full bundle evolution
    report is required.
    ...
    ``after_root``, ``before_ids``, ``after_ids``, ``digest``; both id
    arrays must be non-empty lists of unique non-empty strings (caller
    order is arbitrary; comparison is by code point). Any structural,
    value-domain, uniqueness, ordering, or change-consistency violation
    raises ``ValueError``, and both inputs finish structural validation
    before any comparison.

    Only afterwards are values recomputed rather than trusted: every
    embedded window proof (window boundaries, anchor, embedded bundles,
    adjacent diffs, and the stage chain through its commitment), both
    bundle member chains and roots, the full change set, and the diff
    digest over the five declared prefix items in their canonical
    compact-JSON form. Within this single top-level call each deep window
    proof is normalized and fully recomputed only once; later comparison,
    assembly, and digesting reuse that result. Returns ``False`` when a
    proof, chain link, bundle root, the change set, the diff digest, or an
