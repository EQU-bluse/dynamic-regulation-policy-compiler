...
def _checkpoint_chain_bundle_evolution_stage_digest(
    previous: str, at: str, bundle: dict[str, Any], diff: dict[str, Any] | None
) -> str:
...
def _chain_checkpoint_bundle_semantics(bundle: dict[str, Any]) -> list[tuple[str, Any]]:
    """Summary-free semantic projection of a canonical chain proof bundle.

    Projects a ``root, proofs`` chain window proof bundle to the ordered
    list of member ids and their summary-free window proof semantics, so a
    bundle embedded inside a stage diff can be checked against the
    neighboring stage bundle on semantics: a false member or root summary
    then yields ``False`` at recomputation instead of a structural
    ``ValueError``.
    """
    return [
...
def checkpoint_chain_checkpoint_bundle_evolution(stages: Any) -> dict[str, Any]:
    """Chain checkpoint chain window proof bundles across release stages.
    ...
    empty list, duplicates, disorder, or missing/extra fields raise
    ``ValueError``), and every ``bundle`` must pass every existing
    :func:`checkpoint_chain_checkpoint_bundle` structural, embedded window
    proof, member-chain, and root check — a false window proof, member
    link, or root digest raises ``ValueError``.

    Returns a deep copy with keys ``root``, ``stages`` in that order; each
    stage has keys ``at``, ``bundle``, ``diff``, ``previous``, ``digest``.
    The first stage's ``diff`` is ``null``; every later ``diff`` is the
    complete :func:`checkpoint_chain_checkpoint_bundle_diff` credential
    from the preceding bundle to the current one under the existing diff
    semantics. The first stage's ``previous`` is 64 ASCII ``0`` characters;
    every later stage's ``previous`` is the preceding stage's ``digest``.
    The per-stage payload contains exactly ``at``, ``bundle``, ``diff`` in
    that key order, encoded as compact UTF-8 JSON (``ensure_ascii=False``,
    ``separators=(',', ':')``) with no newline; ``digest`` is the lowercase
    hexadecimal SHA-256 of ``previous``'s ASCII bytes immediately followed
    by that payload, and ``root`` is the last stage's ``digest``.
    Equal-valued inputs yield byte-identical results and no level shares an
    ...
    for index, stage in enumerate(stages):
        ...
        # Every bundle must fully verify (structure, embedded window proofs,
        # member chain, and root) before anything is produced.
        bundle_root, bundle_members = (
            _normalize_verified_checkpoint_chain_checkpoint_bundle(
                stage["bundle"], f"{field}.bundle"
    ...
    for at, bundle in normalized:
        ...
        digest = _checkpoint_chain_bundle_evolution_stage_digest(
...
def _normalize_verified_checkpoint_chain_bundle_evolution(
    report: Any, field: str = "report"
) -> tuple[str, list[dict[str, Any]]]:
    """Run every structural and semantic check on a chain bundle evolution.

    Enforces the full :func:`checkpoint_chain_checkpoint_bundle_evolution`
    report contract: exact top-level and per-stage key sets, hex64
    ``root``/``previous``/``digest`` fields, a non-empty strictly increasing
    ``at`` chain, fully valid chain window proof bundles, a ``null`` diff on
    the first stage and a complete structurally valid adjacent chain bundle
    diff credential on every later stage whose embedded bundles reproduce
    the neighboring stage bundles on semantics. Returns ``(root, stages)``
    with fresh normalized entries (canonical bundle and diff copies). A
    false declared bundle summary, broken bundle chain, or incomplete legal
    change list is carried back as data so it yields ``False`` only after
    every input validates. Any structural or semantic violation raises
    ``ValueError`` and the input is never mutated.
    """
    if not isinstance(report, dict) or set(report) != set(
...
def verify_checkpoint_chain_checkpoint_bundle_evolution(
    ...
    """Verify a chain window proof bundle evolution report by recomputation.
    ...
    Only afterwards are values recomputed rather than trusted: every
    bundle's embedded window proofs and member chain, each adjacent
    :func:`checkpoint_chain_checkpoint_bundle_diff` credential (its digest
    included), every stage digest over the ``previous`` bytes plus the
    compact JSON of ``at``, ``bundle``, ``diff``, the stage
    ``previous``/``digest`` chain from the 64-zero genesis, the final
    ``root``, and the ``expected`` timestamp/bundle-root/diff-digest
    ...
    stage is skipped; otherwise ``True``.
    """
    report_root, stages = _normalize_verified_checkpoint_chain_bundle_evolution(
        report, "report"
    )
    ...
    for index, stage in enumerate(stages):
        ...
        if index > 0:
            ...
            if recomputed_diff["digest"] != expected_diff_digest:
                return False
        recomputed_digest = _checkpoint_chain_bundle_evolution_stage_digest(
            previous, stage["at"], stage["bundle"], stage["diff"]
        )
        if stage["previous"] != previous:
            return False
