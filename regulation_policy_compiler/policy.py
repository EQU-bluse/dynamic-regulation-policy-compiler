...
def _validate_cases(cases: Any) -> list[dict[str, Any]]:
    ...
    for index, case in enumerate(cases):
        ...
        if not isinstance(case, dict) or set(case) != _CASE_KEYS:
            raise ValueError(
                f"{field} must be a dict with exactly the keys id, facts"
            )
...
def decision_impact(
    ...
    """Combine a policy diff with per-case decision changes across two times.
    ...
    exactly as in :func:`explain`. Any violation raises ``ValueError``
    without mutating the inputs. An empty ``cases`` list is valid.

    Returns a deep copy with keys ``from``, ``to``, ``policy``, ``cases``.
    ``policy`` is the full :func:`policy_delta` result for the two
    timestamps. ``cases`` is sorted by ``id`` in Unicode code point order;
    each entry has keys ``id``, ``changes``, ``before``, ``after`` where
    ``before``/``after`` are the full :func:`explain` results at
    ``from_at``/``to_at`` for that case's facts, and ``changes`` lists, in
    the order ``decision``, ``trace``, ``basis``, ``conflicts``, the fields
    whose values differ between the two snapshots (empty when identical).
    """
    _check_time(from_at, "from_at")
    _check_time(to_at, "to_at")
    if from_at > to_at:
