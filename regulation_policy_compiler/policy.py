...
_CHAIN_CHECKPOINT_BUNDLE_DIFF_REPORT_KEYS = (
    "before_root",
    "after_root",
    "before",
    "after",
...
_CHAIN_CHECKPOINT_BUNDLE_DIFF_CHANGE_KEYS = frozenset(
    {"id", "kind", "before", "after"}
)
_CHAIN_CHECKPOINT_BUNDLE_DIFF_EXPECTED_KEYS = (
    "before_root",
    "after_root",
    "before_proof_ids",
    "after_proof_ids",
...
_CHAIN_CHECKPOINT_BUNDLE_DIFF_KINDS = frozenset({"added", "removed", "changed"})


def _chain_checkpoint_diff_semantics(diff: dict[str, Any] | None) -> Any:
    """Summary-free semantic projection of a stage diff in a chain window proof.

    Mirrors :func:`_checkpoint_diff_semantics`, but the stage diff embedded
    in a chain window proof is an
    :func:`evolution_checkpoint_bundle_diff`: its before/after sides are
    checkpoint bundles and each non-null change side is an evolution
    checkpoint proof. Every digest, bundle root, and hash field inside it is
    a derived summary, not semantics.
    """
    if diff is None:
        return None
    ...
    def proof_semantics(proof: dict[str, Any] | None) -> Any:
        if proof is None:
            return None
        return _checkpoint_proof_semantics(proof)
...
def _chain_checkpoint_proof_semantics(proof: dict[str, Any]) -> dict[str, Any]:
    ...
    Mirrors :func:`_checkpoint_proof_semantics` for the
    :func:`checkpoint_chain_checkpoint` proof shape: stage bundles are
    checkpoint bundles (projected with
    :func:`_checkpoint_bundle_semantics`) and stage diffs are checkpoint
    bundle diffs (projected with
    :func:`_chain_checkpoint_diff_semantics`). The proof's
    ``anchor``/``commitment``, every stage ``previous``/``digest``, and all
    embedded summaries are derived, so two window proofs over the same
    window, bundles, and transitions describe the same member even when
    their hash fields differ.
    """
    return {
        "start": proof["start"],
        "end": proof["end"],
        "stages": [
            {
                ...
                "diff": _chain_checkpoint_diff_semantics(stage["diff"]),
            }
            for stage in proof["stages"]
        ],
    }
...
def _chain_checkpoint_bundle_is_truthful(
    ...
    """Recompute a structurally normalized chain proof bundle's hash chain.
...
def _canonical_chain_checkpoint_bundle_from_members(
    bundle_root: str, members: list[dict[str, Any]]
) -> dict[str, Any]:
    """Rebuild a fresh ``root, proofs`` chain proof bundle from members."""
    return {
        "root": bundle_root,
        "proofs": [
            {
...
def checkpoint_chain_checkpoint_bundle_diff(before: Any, after: Any) -> dict[str, Any]:
    ...
    ordered by ``id`` in ascending Unicode code point order. Two proofs are
    the same member when their summary-free semantics — window boundaries,
    stage times, embedded bundles, and adjacent transitions — are equal, so
    differing but recomputable hash summaries alone never fabricate a
    ``changed`` entry. Each change has keys ``id``, ``kind``, ``before``,
    ``after``; the two sides are deep copies of the window proof or ``null``
    (an added entry's ``before`` and a removed entry's ``after`` are null).
    ...
    """
    before_root, before_members = (
        _normalize_verified_checkpoint_chain_checkpoint_bundle(before, "before")
    )
    after_root, after_members = (
    ...
    before_bundle = _canonical_chain_checkpoint_bundle_from_members(
        before_root, before_members
    )
    after_bundle = _canonical_chain_checkpoint_bundle_from_members(
        after_root, after_members
    )
    before_by_id = {member["id"]: member for member in before_members}
    after_by_id = {member["id"]: member for member in after_members}
    ...
    for member_id in sorted(set(before_by_id) | set(after_by_id)):
        ...
        if (
            before_member is not None
            and after_member is not None
            and _chain_checkpoint_proof_semantics(before_member["proof"])
...
def _normalize_chain_diff_change_proof(
    value: Any, field: str
) -> dict[str, Any] | None:
...
def _normalize_verified_chain_checkpoint_diff_changes(
    ...
    """Structurally validate a chain proof bundle diff ``changes`` list.
    ...
    for index, change in enumerate(value):
        item_field = f"{field}[{index}]"
        if not isinstance(change, dict) or set(change) != (
            _CHAIN_CHECKPOINT_BUNDLE_DIFF_CHANGE_KEYS
        ):
            raise ValueError(
                f"{item_field} must be a dict with exactly the keys "
                "id, kind, before, after"
        ...
        kind = change["kind"]
        if kind not in _CHAIN_CHECKPOINT_BUNDLE_DIFF_KINDS:
            raise ValueError(
                f"{item_field}.kind must be 'added', 'removed', or 'changed'"
            )
        before_proof = _normalize_chain_diff_change_proof(
            change["before"], f"{item_field}.before"
        )
        after_proof = _normalize_chain_diff_change_proof(
            change["after"], f"{item_field}.after"
        )
        before_member = before_by_id.get(member_id)
        after_member = after_by_id.get(member_id)
    ...
    for member_id in sorted(set(before_by_id) & set(after_by_id)):
        # Membership in the diff is a semantic question; false summaries are
        # recomputed afterwards and must not fabricate "changed" members.
        if _chain_checkpoint_proof_semantics(before_by_id[member_id]["proof"]) != (
...
def _normalize_verified_chain_checkpoint_diff_report(
    ...
    bundles, both bundles' full structural and embedded-proof checks, and
    the per-item change consistency rules. Returns ``(content,
    before_members, after_members, change_entries, changes_complete)`` where
    ``content`` is a fresh canonical-order dict with the six diff keys (its
    ``before``/``after`` bundles and change proofs are independent deep
    copies) and ``changes_complete`` records whether the legal change list
    covers exactly the added, removed, and changed members. False summaries
    and incomplete change lists are not detected here — callers recompute
    them afterwards, yielding ``False`` rather than ``ValueError``.
    ...
    content = {
        "before_root": before_root,
        "after_root": after_root,
        "before": _canonical_chain_checkpoint_bundle_from_members(
            before_root, before_members
        ),
        "after": _canonical_chain_checkpoint_bundle_from_members(
            after_root, after_members
        ),
        "changes": [
            {
...
def verify_checkpoint_chain_checkpoint_bundle_diff(
    ...
    embedded window proof is self-verifying, so no outside chain is needed.
    ...
    root must equal the root declared by its embedded bundle. ``before``/
    ``after`` must be complete chain window proof bundles that pass every
    structural, member-order, digest-format, and embedded window-proof
    semantic check. ``changes`` items must have exactly the keys ``id``,
    ``kind``, ``before``, ``after`` with non-empty unique strictly ascending
    ids, legal kinds, null sides matching each kind, and non-null sides that
    are valid window proofs exactly reproducing the corresponding bundle
    members. A structurally legal change list that omits an expected change
    or lists an extra (or unchanged) one is not a structural error: it
    yields ``False`` after validation.
    ``expected`` must have exactly the keys ``before_root``, ``after_root``,
    ``before_proof_ids``, ``after_proof_ids``, ``digest``; both id arrays
    must be non-empty lists of unique non-empty strings (caller order is
    arbitrary; comparison is by code point). Any structural, value-domain,
    uniqueness, ordering, or change-consistency violation raises
    ``ValueError``, and both inputs finish structural validation before any
    comparison.
    ...
    bundle member chains and roots, the full set of added, removed, and
    changed members (proofs compared on their summary-free semantics), and
    the diff digest over the five declared prefix items in their canonical
    compact-JSON form. Returns ``False`` when a proof, chain link, bundle
    root, the change set, the diff digest, or an ``expected`` value (the two
    roots, the digest, or either id set after code point sorting) does not
    match; otherwise ``True``.
    """
    content, before_members, after_members, changes, changes_complete = (
    ...
    if not isinstance(expected, dict) or set(expected) != set(
        ...
        raise ValueError(
            "expected must be a dict with exactly the keys "
            "before_root, after_root, before_proof_ids, after_proof_ids, digest"
        )
    expected_before_root = _check_hex64(
        expected["before_root"], "expected.before_root"
    )
    ...
    expected_digest = _check_hex64(expected["digest"], "expected.digest")
    expected_before_ids = _validate_matrix_bundle_report_ids(
        expected["before_proof_ids"], "expected.before_proof_ids"
    )
    expected_after_ids = _validate_matrix_bundle_report_ids(
        expected["after_proof_ids"], "expected.after_proof_ids"
    )
    ...
    for change in changes:
        for proof in (change["before"], change["after"]):
            ...
            # window, anchor, embedded bundles and diffs, and stage chain
            # through its commitment must all recompute as true.
            if not _checkpoint_chain_checkpoint_is_truthful(proof):
                return False
    payload = {
        key: content[key] for key in _CHAIN_CHECKPOINT_BUNDLE_DIFF_REPORT_KEYS[:5]
    }
    canonical = json.dumps(payload, ensure_ascii=False, separators=(",", ":"))
    recomputed_digest = hashlib.sha256(canonical.encode("utf-8")).hexdigest()
    if digest != recomputed_digest:
