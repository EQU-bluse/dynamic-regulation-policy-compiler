...
def bundle_evolution_checkpoint(
    ...
    The proof covers the continuous run of stages from ``start`` through
    ``end`` inclusive, with no stage skipped, reordered, replaced, or
    rewritten. Returns a deep copy containing exactly the keys ``start``,
    ``end``, ``anchor``, ``stages``, ``commitment`` in that order. When the
    window begins at the report's first stage, ``anchor`` is 64 ASCII ``0``
    characters; otherwise it is the immediately preceding stage's
    ``digest``. ``stages`` are independent deep copies of the full window
    stages, each still following the published window-proof-bundle, diff,
    and stage-chain contracts; the first stage of a non-head window keeps
    its original ``diff`` and the bundle on its front side, so even a
    ...
    equals the original report ``root`` when the window reaches the
    report's end. Equal-valued inputs yield byte-identical results.
    """
    start = _check_time(start, "start")
    end = _check_time(end, "end")
    if start > end:
...
def _normalize_verified_bundle_evolution_checkpoint(
    ...
    diff) or carries the full front-side bundle diff whose ``after`` bundle
    reproduces the first stage. Window disorder, an illegal change item, or
    an adjacent diff that does not correspond raises ``ValueError``. False
    declared summaries are carried back as data (``bundle_truthful``,
    ``diff_complete``, and the validated carried-diff members) so they
...
def verify_bundle_evolution_checkpoint(proof: Any, expected: Any) -> bool:
    ...
    and a ``null`` diff; any other first stage keeps its front-side bundle
    diff whose ``after`` bundle reproduces that stage, and every later diff
    must reproduce the preceding and current bundles. ``expected`` must have
    exactly the keys ``start``, ``end``, ``anchor``, ``commitment`` and is
    validated the same way; no other fields are accepted. Exact key sets,
    UTC second times, digest formats, stage order, and every embedded
    credential semantic are checked on both inputs first; an illegal
    structure, an invalid bundle, a wrongly classified change, or an
    adjacent diff that does not correspond raises ``ValueError``.

    Only afterwards are values recomputed rather than trusted: the first
    stage is recomputed from ``anchor`` (a non-head first stage's carried
    diff recomputed together with its front-side bundle's member chain and
    root), then every bundle member chain, each adjacent
    :func:`checkpoint_chain_checkpoint_bundle_diff` credential, every
    ``previous``/``digest`` stage link, and the terminal ``commitment``.
    Returns ``False`` for a wrong anchor, a broken link, a skipped or
    extra-listed change, a boundary or ``expected`` disagreement after the
